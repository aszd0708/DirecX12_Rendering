...
{
	static const UINT32 DEFAULT_MEMORY_HANDLER_ARRAY_SIZE = 32;
...
public:
	GpuDynamicMemoryPool(UINT8 poolID, bool using4MBSize);
	GpuDynamicMemoryPool(UINT8 poolID, UINT64 size, bool using4MBSize);
	~GpuDynamicMemoryPool();

private:
	Array<GpuDynamicMemoryPage*> ComputePageCount(bool using4MBSize);
	void CreateHeap();
...
public:
	...
	bool ReleaseMemoryHandle(eGpuMemoryPoolType type, const GpuMemoryHandle& handle);
